...
| `content/noticed.ts` | 03 — Things I Noticed |
...
**Every "Things I Noticed" card must be something she'd be glad was noticed.**
Character, never habits, appearance, or movements. The rule is written into
`content/noticed.ts` so it survives your edits.
...
Rain and a sparse piano, synthesised in the browser with WebAudio. No audio
files, so nothing to load and no audible loop point. It never autoplays; the
`AudioContext` isn't even constructed until she presses the speaker.
