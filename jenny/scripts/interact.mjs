...
check("final beat present", await page.getByText("You're reading this.").isVisible());
...
check("scan verdict appears", await page.getByText("No shortcuts detected.").isVisible());
check(
...
check(
  "long drive fires the PRD reaction",
  await page.getByText(/Bonus points if it's raining/i).isVisible(),
);
